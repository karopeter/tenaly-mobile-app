...
export default function PostGadgetForm() {
    ...
    const getDropdownData = () => {
        ...
        const baseDropdowns: Record<string, string[]> = { 
           ...
           storageCapacity: ['8 GB', '16 GB', '32 GB', '64 GB', '128 GB', '256 GB', '512 GB', '1 TB'],
           ram: ['1 GB', '2 GB', '4 GB', '8 GB', '16 GB', '32 GB'],
           operatingSystem: ['Andriod', 'iOS', 'HarmonyOS', 'Windows'],
           network: ['2G', '3G', '4G', '5G'],
            simType: ['Single SIM', 'Dual SIM', 'eSIM'],
        };

        if (categoryStr.includes('Tablet Accessories')) {
           return {
            ...baseDropdowns
           };
        }
         ...
         if (!draftMode || !draftCarAdId) {
              return;
                  ...
                  try {
                   ...
                   // Fetch Fashion Draft 
                   const gadgetResponse = await apiClient.get(`/api/gadget/draft/${draftCarAdId}`);
                   const gadgetAd = gadgetResponse.data.gadgetAd;
          
                   setFormData({
                    gadgetTitle: gadgetAd.gadgetTitle || '',
                    condition: gadgetAd.condition || '',
                    gadgetBrand: gadgetAd.gadgetBrand || '',
                    storageCapacity: gadgetAd.storageCapacity || '',
                    ram: gadgetAd.ram || '',
                    operatingSystem: gadgetAd.operatingSystem || '',
                    simType: gadgetAd.simType || '',
                    network: gadgetAd.network || '',
                    batteryHealth: gadgetAd.batteryHealth || '',
                    gadgetColor: gadgetAd.gadgetColor || '',
                    accessories: gadgetAd.accessories || '',
                    warranty: gadgetAd.warranty || '',
                    amount: gadgetAd.amount || '',
                    negotiation: gadgetAd.negotiation || '',
                    description: gadgetAd.description || '',
                   });
          
                   // Set If business is available 
                   if (gadgetAd.businessCategory) {
                    setSelectectBusiness({
                      _id: gadgetAd.businessCategory._id || gadgetAd.businessCategory,
                      businessName: gadgetAd.businessCategory.businessName || 'Selected Business'
                    });
                   }
   ...
   const fetchUserProfile = async () => {
    ...
    const submitAd = async (plan: string, useWallet: boolean) => {
        try {
         ...
         const payload = {
          gadgetTitle: formData.gadgetTitle,
          condition: formData.condition,
          gadgetBrand: formData.gadgetBrand,
          ...
          gadgetColor: formData.gadgetColor,
         ...
         const response = await apiClient.post('/api/gadget/create-gadget-ad', payload);

         console.log('✅ Response:', response.data);

         if (response.data.data.paymentStatus === 'success') {
            showSuccessToast('Gadget ad posted successfully!');
 ...
 const startPaymentPolling = (reference: string) => {
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/gadget/verify-payment/${reference}`);
...
const handleSubmit = async () => {
   ...
   if (!formData.gadgetTitle || !formData.condition || !formData.gadgetBrand || !formData.amount) {
     showErrorToast('Please fill all required fields (gadgetTitle, condition, gadgetBrand, amount)');
     return;
   }
 ...
 const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/api/gadget/save-draft', payload);
    ...
    <View style={styles.container}>
     <View style={styles.header}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>
          {isDraftMode ? 'Complete Post Gadget Ad' : 'Post Gadget Ad'}
        </Text>
      </View>
     </View>
     ...
     >
       ...
       <View style={styles.row}>
          <PostAdInput
            label="Title"
            value={formData.gadgetTitle}
            placeholder='Enter Title'
            onChangeText={(value) => updateField('gadgetTitle', value)}
          />
       </View>
      ...
      <View style={styles.row}>
       <PostAdInput
         ...
         onChangeText={(value) => updateField('gadgetBrand', value)}
       />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          ...
          onPress={() => openDropdown('storageCapacity', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          label="RAM (optional)"
          value={formData.ram}
          onPress={() => openDropdown('ram', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
       <PostAdDropdown
         ...
         onPress={() => openDropdown('operatingSystem', 'Select')}
       />
      </View>
      ...
      <View style={styles.row}>
       <PostAdDropdown
         ...
         onPress={() => openDropdown('simType', 'Select')}
       />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          ...
          onPress={() => openDropdown('network', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
        <PostAdInput
          ...
          placeholder="e.g 500mAh or 92%"
          value={formData.batteryHealth}
          onChangeText={(value) => updateField('batteryHealth', value)}
        />
      </View>
       ...
       <View style={styles.row}>
          <PostAdInput
           ...
           value={formData.gadgetColor}
           onChangeText={(value) => updateField('gadgetColor', value)}
          />
       </View>
