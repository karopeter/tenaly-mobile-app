...
export default function PostFashionAccessoriesForm() {
    ...
    const getDropdownData = () => {
        ...
        if (categoryStr.includes('Eyewear (Glasses & Sunglasses)')) {
           return {
            ...baseDropdowns
           };
        }
    ...
    useEffect(() => {
      const fetchDraftData = async () => {
        ...
        try {
         ...
         // Fetch Fashion Draft 
         const fashionResponse = await apiClient.get(`/api/fashion/draft/${draftCarAdId}`);
         const fashionAd = fashionResponse.data.fashionAd;

         setFormData({
           fashionTitle: fashionAd.fashionTitle || '',
           fashionType: fashionAd.fashionType || '',
           condition: fashionAd.condition || '',
           fashionBrand: fashionAd.fashionBrand || '',
           frameMaterial: fashionAd.frameMaterial || '',
           gender: fashionAd.gender || '',
           lensType: fashionAd.lensType || '',
           fashionColor: fashionAd.fashionColor || '',
           frameShape: fashionAd.frameShape || '',
           fashionAccessories: fashionAd.fashionAccessories || '',
           amount: fashionAd.amount || '',
           negotiation: fashionAd.negotiation || '',
           description: fashionAd.description || '',
         });

         // Set If business is available 
         if (fashionAd.businessCategory) {
          setSelectectBusiness({
            _id: fashionAd.businessCategory._id || fashionAd.businessCategory,
            businessName: fashionAd.businessCategory.businessName || 'Selected Business'
          });
         }
    ...
    const submitAd = async (plan: string, useWallet: boolean) => {
        try {
         ...
         const payload = {
           ...
           fashionBrand: formData.fashionBrand,
           frameMaterial: formData.frameMaterial,
           gender: formData.gender,
           lensType: formData.lensType,
           fashionColor: formData.fashionColor,
           frameShape: formData.frameShape,
           fasionAccessories: formData.fashionAccessories,
         ...
         const response = await apiClient.post('/api/fashion/create-fashion-ad', payload);

         console.log('✅ Response:', response.data);

         if (response.data.data.paymentStatus === 'success') {
            showSuccessToast('Fashion ad posted successfully!');
 ...
 const startPaymentPolling = (reference: string) => {
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/fashion/verify-payment/${reference}`);
...
const handleSubmit = async () => {
   ...
   if (!formData.fashionTitle|| !formData.condition || !formData.amount) {
     showErrorToast('Please fill all required fields (fashionTitle, condition,  amount)');
     return;
   }
 ...
 const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const payload = {
       ...
       fashionColor: formData.fashionColor,
       frameShape: formData.frameShape,
       fashionAccessories: formData.fashionAccessories,
        amount: formData.amount,
        negotiation: formData.negotiation,
        description: formData.description,
      ...
      const response = await apiClient.post('/api/fashion/save-draft', payload);
    ...
    <View style={styles.container}>
     <View style={styles.header}>
      ...
      <Text style={styles.headerTitle}>
        {isDraftMode ? 'Complete Post Eyewear Ad' : 'Post Eyewear Ad'}
      </Text>
      </View>
     </View>
     ...
     >
       ...
       <View style={styles.row}>
          <PostAdInput
            ...
            onChangeText={(value) => updateField('fashionTitle', value)}
          />
       </View>
       ...
       <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('fashionType', 'Select')}
         />
       </View>
      ...
      <View style={styles.row}>
       <PostAdInput
         ...
         value={formData.fashionBrand}
         placeholder='E.g Ray-Ban, Gucci, Prada, Dior, Polaroid'
         onChangeText={(value) => updateField('fashionBrand', value)}
       />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          ...
          onPress={() => openDropdown('frameMaterial', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('gender', 'Select')}
         />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          ...
          onPress={() => openDropdown('lensType', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
        <PostAdInput
          label="Color"
          value={formData.fashionColor}
          placeholder="Enter"
          onChangeText={(value) => updateField('fashionColor', value)}
        />
      </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          ...
          onPress={() => openDropdown('frameShape', 'Select')}
        />
      </View>
      ...
      <View style={styles.row}>
        <PostAdInput
          ...
          value={formData.fashionAccessories}
          onChangeText={(value) => updateField('fashionAccessories', value)}
        />
      </View>
