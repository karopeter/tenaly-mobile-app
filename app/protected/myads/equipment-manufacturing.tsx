...
export default function PostEquipmentManufacturingForm() {
   ...
   const [isDraftMode, setIsDraftMode] = useState(false);
    ...
    const getDropdownData = () => {
        ...
        const baseDropdowns: Record<string, string[]> = { 
            negotiation: ["Yes", "No"],
           condition: ['Brand New', 'UK/Foreign Used', 'Nigerian Used',],
            equipmentType: [
              "Bulldozer",
              "Compactor",
              "Excavator",
        ...
        if (categoryStr.includes('Manufacturing Equipment')) {
           return {
            ...baseDropdowns
           };
        }
      ...
      useEffect(() => {
            const fetchDraftData = async () => {
              ...
              try {
               ...
               // Fetch Equipment draft 
               const equipmentResponse = await apiClient.get(`/api/equipments/draft/${draftCarAdId}`);
               const equipmentAd = equipmentResponse.data.equipementAd;
      
               // Pre-fill form data 
               setFormData({
                equipmentTitle: equipmentAd.equipmentTitle || '',
                equipmentType: equipmentAd.equipmentType || '',
                condition: equipmentAd.condition || '',
                powerSource: equipmentAd.powerSource || '',
                brand: equipmentAd.brand || '',
                usageType: equipmentAd.usageType || '',
                yearOfManufacture: equipmentAd.yearOfManufacture || '',
                fuelType: equipmentAd.fuelType || '',
                amount: equipmentAd.amount || '',
                negotiation: equipmentAd.negotiation || '',
                description: equipmentAd.description || '',
               });
      
               // Set business if available 
               if (equipmentAd.businessCategory) {
                setSelectectBusiness({
                  _id: equipmentAd.businessCategory._id || equipmentAd.businessCategory,
                  businessName: equipmentAd.businessCategory.businessName || 'Selected Business'
                });
               }
   ...
   const fetchUserProfile = async () => {
    ...
    const submitAd = async (plan: string, useWallet: boolean) => {
        try {
         ...
         const response = await apiClient.post('/api/equipments/create-equipment-ad', payload);

         console.log('✅ Response:', response.data);

         if (response.data.data.paymentStatus === 'success') {
            showSuccessToast('Equipment ad posted successfully!');
 ...
 const startPaymentPolling = (reference: string) => {
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/equipments/verify-payment/${reference}`);
...
const handleSubmit = async () => {
   ...
   if (!formData.equipmentTitle|| !formData.condition || !formData.amount) {
     showErrorToast('Please fill all required fields (equipmentTitle condition,  amount)');
     return;
   }
 ...
 const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/api/equipments/save-draft', payload);
     ...
     <View style={styles.header}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>
          {isDraftMode ? 'Complete Post Work Tools & Machinery Ad' : 'Post Work Tools & Machinery Ad'}
        </Text>
      </View>
     </View>
     ...
     >
       ...
       <View style={styles.row}>
          <PostAdInput
            ...
            onChangeText={(value) => updateField('equipmentTitle', value)}
          />
       </View>
       ...
       <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('equipmentType', 'Select')}
         />
       </View>
       ...
       <View style={styles.row}>
          <PostAdDropdown
           ...
           onPress={() => openDropdown('powerSource', 'Select')}
          />
       </View>
       ...
       <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('usageType', 'Select')}
         />
       </View>
        ...
        <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('fuelType', 'Select')}
         />
        </View>
