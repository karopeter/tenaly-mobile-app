...
export default function PostKidFurnitureForm() {
    ...
    const getDropdownData = () => {
        ...
        const baseDropdowns: Record<string, string[]> = { 
           ...
           kidsMaterial: ['Wood', 'Metal', 'Plastic', 'Others'],
           gender: ['Male', 'Female', 'Unisex'],
        };

        if (categoryStr.includes('Furniture (cribs, cots, wardrobes)')) {
           return {
            ...baseDropdowns
           };
        }
    ...
    useEffect(() => {
      ...
      const fetchDraftData = async () => {
        ...
        try {
         ...
         setFormData({
          title: kidAd.title || '',
          kidsType: kidAd.kidsType || '',
          condition: kidAd.condition || '',
          kidsMaterial: kidAd.kidsMaterial || '',
          color: kidAd.color || '',
          gender: kidAd.gender || '',
          amount: kidAd.amount || '',
    ...
    const submitAd = async (plan: string, useWallet: boolean) => {
        try {
         ...
         const payload = {
           ...
           kidsType: formData.kidsType,
           condition: formData.condition,
           kidsMaterial: formData.kidsMaterial,
         ...
         const response = await apiClient.post('/api/kids/create-kids-ad', payload);

         console.log('✅ Response:', response.data);

         if (response.data.data.paymentStatus === 'success') {
            showSuccessToast('Kid ad posted successfully!');
 ...
 const startPaymentPolling = (reference: string) => {
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/kids/verify-payment/${reference}`);
...
const handleSubmit = async () => {
   ...
   if (!formData.title || !formData.condition || !formData.gender || !formData.amount) {
     showErrorToast('Please fill all required fields (title, condition, gender, amount)');
     return;
   }
 ...
 const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/api/kids/save-draft', payload);
     ...
     <View style={styles.header}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>
          {isDraftMode ? 'Complete Post For baby Furniture Ad' : 'Post For baby Furniture Ad'}
        </Text>
      </View>
     </View>
     ...
     >
       ...
       <View style={styles.row}>
        <PostAdDropdown 
           ...
           onPress={() => openDropdown('kidsType', 'Select')}
        />
       </View>
      ...
      <View style={styles.row}>
        <PostAdDropdown
          label="Material"
          value={formData.kidsMaterial}
          onPress={() => openDropdown('kidsMaterial', 'Select')}
        />
      </View>
