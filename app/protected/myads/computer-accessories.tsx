...
export default function PostComputerAccessoriesForm() {
    ...
    const getDropdownData = () => {
        ...
        if (categoryStr.includes('Computer Accessories')) {
           return {
            ...baseDropdowns
           };
        }
    ...
    const submitAd = async (plan: string, useWallet: boolean) => {
        try {
         ...
         const payload = {
          ...
          laptopConnectivityType: formData.laptopConnectivityType ? formData.laptopConnectivityType.split(', ') : [],
 ...
 const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const payload = {
         ...
         laptopBrand: formData.laptopBrand,
         capacity: formData.capacity,
         laptopConnectivityType: Array.isArray(formData.laptopConnectivityType)
          ? formData.laptopConnectivityType
          : formData.laptopConnectivityType
          ? formData.laptopConnectivityType.split(", ").map((f) => f.trim())
          : [],
        warranty: formData.warranty,
        amount: formData.amount,
        negotiation: formData.negotiation,
     ...
     >
      ...
      <View style={styles.row}>
        <MultiSelectDropdown
          ...
          selectedValues={formData.laptopConnectivityType ? formData.laptopConnectivityType.split(', '): []}
          options={[
            'Wired',
            'Wireless',
            'Bluetooth',
          ...
          onChange={(values) => 
            updateField('laptopConnectivityType', values.join(', '))
          }
        />
      </View>
