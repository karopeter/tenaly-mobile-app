...
export default function PostBeautySkinForm() {
   ...
   const getDropdownData = () => {
    ...
    const baseDropdowns: Record<string, string[]> = {
      ...
      condition: ['Brand New', 'Used (Open Pack)'],
      beautyType: ['Face Cream', 'Body Lotion', 'Sunscreen', 'Serum', 'Cleanser', 'Moisturizer', 'Scrub', 'Face Mask', 'Body Oil', 'Spot Treatment', 'Others'],
      skinType: ['All Skin Types', 'Dry', 'Sensitive', 'Normal', 'Oily', 'Combination'],
      targetConcern: ['Acne', 'Hyperpigmentation', 'Wrinkles', 'Dark Spot', 'Sunburn', 'Strecth Marks'],
    };

    if (categoryStr.includes('Skin Care')) {
      return {
        ...baseDropdowns
      };
    }
     ...
     const fetchDraftData = async () => {
       ...
       try {
        ...
        setFormData({
          ...
          skinType: beautyAd.skinType || '',
          gender: beautyAd.gender || '',
          targetConcern: beautyAd.targetConcern || '',
  ...
  const submitAd = async (plan: string, useWallet: boolean) => {
     try {
      ...
      const payload = {
         ...
         beautyType: formData.beautyType,
         condition: formData.condition,
         skinType: formData.skinType,
         gender: formData.gender,
         targetConcern: formData.targetConcern,
         beautyBrand: formData.beautyBrand,
      ...
      console.log('📤 Submitting beauty ad:', payload);

      const response = await apiClient.post('/api/beauty/create-beauty-ad', payload);

      console.log('✅ Response:', response.data);

      if (response.data.data.paymentStatus === 'success') {
        showSuccessToast('Beauty ad posted successfully!');
...
const startPaymentPolling = (reference: string) => {
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/beauty/verify-payment/${reference}`);
 ...
 const handleSubmit = async () => {
    ...
    if (!formData.beautyTitle || !formData.beautyBrand ||  !formData.gender || !formData.amount) {
      showErrorToast('Please fill all required fields (beautyTitle, beautyBrand, Gender, Amount)');
      return;
    }
 ...
 const handleSaveAsDraft = async () => {
     ...
     try {
      ...
      const response = await apiClient.post("/api/beauty/save-draft", payload);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
         ...
         {/* <Text style={styles.headerTitle}>Post Skincare Ad</Text> */}
         <Text style={styles.headerTitle}>
           {isDraftMode ? 'Complete Skincare Ad' : 'Post Skincare Ad'}
         </Text>
         </View>
      </View>
      ...
      >
        ...
        <View style={styles.row}>
           <PostAdInput
            ...
            onChangeText={(value) => updateField('beautyTitle', value)}
           />
        </View>
        ...
        <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('beautyType', 'Select Type')}
         />
        </View>
        ...
        <View style={styles.row}>
         <PostAdDropdown
           ...
           onPress={() => openDropdown('skinType', 'Select Skin Type')}
         />
        </View>
        ...
        <View style={styles.row}>
           <PostAdDropdown
             ...
             onPress={() => openDropdown('gender', 'Select Gender')}
           />
        </View>
        ...
        <View style={styles.row}>
          <PostAdDropdown 
           ...
           value={formData.targetConcern}
           onPress={() => openDropdown('targetConcern', 'Select Target')}
          />
        </View>
        ...
        <View style={styles.row}>
         <PostAdInput 
           label="Brand"
           value={formData.beautyBrand}
           placeholder='Enter brand'
           onChangeText={(value) => updateField('beautyBrand', value)}
         />
        </View>
