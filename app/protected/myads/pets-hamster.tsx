...
export default function PetHamsterForm() {
 ...
 const getDropdownData = () => {
    ...
    const baseDropdowns: Record<string, string[]> = {
      negotiation: ['Yes', 'No'],
       breed: ['Labrador', 'German Shepherd', 'Golden Retriever', 'Bulldog', 'Beagle', 'Poodle'],
     gender: ['Male', 'Female', 'Others'],
    };

    if (categoryStr.includes('Small Pets (rabbits, hamsters, guinea pigs)')) {
        return {
          ...baseDropdowns,
          breed: ['Labrador', 'German Shepherd', 'Golden Retriever', 'Bulldog', 'Beagle', 'Poodle'],
          healthStatus: ['Vaccinated', 'Not Vaccinated', 'Dewormed', 'Vet Checked', 'Has Allergies'],
          gender: ['Male', 'Female', 'Others'],
        };
    }
 ...
 const fetchUserProfile = async () => {
  ...
  const submitAd = async (plan:string, useWallet: boolean) => {
    try {
      ...
      console.log('📤 Submitting pet ad:', payload);

      const response = await apiClient.post('/api/pets/post-pets-ad', payload);

      console.log('✅ Response:', response.data);

      if (response.data.data.paymentStatus === 'success') {
        showSuccessToast('Pet ad posted successfully!');
  ...
  const pollInterval = setInterval(async () => {
    ...
    try {
      ...
      const verifyResponse = await apiClient.get(`/api/pets/verify-payment/${reference}`);
...
const handleSubmit = async () => {
  ...
  if (!formData.petType || !formData.breed || !formData.gender || !formData.amount) {
    showErrorToast('Please fill all required fields (Pet Type, Breed, Gender, Amount)');
    return;
  }
...
const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const response = await apiClient.post("/api/pets/save-draft", payload);
     ...
     <View style={styles.container}>
       <View style={styles.header}>
        <View style={styles.headerContainer}>
         ...
         <Text style={styles.headerTitle}>
          {isDraftMode ? 'Complete Post Hamster Pet Ad' : 'Post Hamster Pet Ad'}
         </Text>
        </View>
       </View>
       ...
       >
        ...
        <PostAdInput
          ...
          placeholder="Enter"
          onChangeText={(value) => updateField('petType', value)}
        /> 
        </View>
        ...
        <View style={styles.row}>
          <PostAdDropdown
            ...
            onPress={() => openDropdown('breed', 'Select Breed')}
          />
        </View>
