...
interface AddressHourData extends BusinessHourData {
    addressId: string;
    address: string;
    checked: boolean;
}
...
export default function AddBusinessHours() {
  ...
  const [showClosingTimeDropdown, setShowClosingTimeDropdown] = useState<string | null>(null);
   ...
   // For single business (same hours for all addresses) 
   const [singleBusinessHours, setSingleBusinessHours] = useState<BusinessHourData>({
    openingTime: '',
    closingTime: '',
    days: []
   });

   // For multiple businesses or different hours per address 
   const [addressHours, setAddressHours] = useState<AddressHourData[]>([]);

   const isAllBusinesses = id === 'all';
   ...
   const fetchBusinessData = async () => {
      try {
       ...
       if (isAllBusinesses) {
        ...
        initializeAddressHours(businessList);
       } else {
        // handle single business 
        const selectedBusiness = businessList.find((b: BusinessData) => b._id === id);
        if (!selectedBusiness) {
        ...
        initializeAddressHours([selectedBusiness]);
       }
      } catch (error: any) {
  ...
  const initializeAddressHours = (businessList: BusinessData[]) => {
  ...
  const toggleDay = (day: string, addressId?: string) => {
    if (addressId) {
        ...
        // For single business hours 
        setSingleBusinessHours(prev => ({
            ...prev,
  ...
  const updateTime = (addressId: string | null, timeType: 'opening' | 'closing', time: string) => {
    if (addressId) {
      setAddressHours(prev => prev.map(item => {
         if (item.addressId === addressId) {
            return {
  ...
  const validateHours = () => {
    if (business && business.addresses.length === 1) {
        // Single address validation
        if (!singleBusinessHours.openingTime || !singleBusinessHours.closingTime) {
            showErrorToast('Please select opening and closing time');
            return false;
        }
        if (singleBusinessHours.days.length === 0) {
        ...
        for (const addressHour of addressHours) {
            if (!addressHour.openingTime || !addressHour.closingTime) {
              showErrorToast(`Please set opening and closing time for ${addressHour.address}`);
              return false;
            }
        }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (business && business.addresses.length === 1) {
        // Single address - use single business hours
        const hoursData = [{
          openingTime: singleBusinessHours.openingTime,
          closingTime: singleBusinessHours.closingTime,
          days: singleBusinessHours.days
        }];
        
        console.log('Saving single address hours:', { businessHours: hoursData });
        
        await apiClient.put(`/api/business/${business._id}/hours`, {
          businessHours: hoursData
        });
      } else {
        // Multiple addresses - use individual hours for each address
        ...
        console.log('Saving multiple address hours:', { businessHours: hoursData });
        
        if (isAllBusinesses) {
          // Save same hours for all businesses
          for (const businessItem of businesses) {
            const businessHoursData = businessItem.addresses.map(() => ({
              openingTime: addressHours[0]?.openingTime || '',
              closingTime: addressHours[0]?.closingTime || '',
              days: addressHours[0]?.days || []
            }));
            
            await apiClient.put(`/api/business/${businessItem._id}/hours`, {
              businessHours: businessHoursData
            });
          }
        } else {
          // Save for single business with multiple addresses
          await apiClient.put(`/api/business/${business?._id}/hours`, {
            businessHours: hoursData
          });
        }
      ...
      showSuccessToast('Business hours saved successfully!');
      setTimeout(() => {
      ...
      showErrorToast(error.response?.data?.message || 'Failed to save business hours');
    } finally {
      setSaving(false);
    }
  };
      ...
      <View style={styles.dropdownOverlay}>
        ...
        <View style={styles.dropdownContent}>
          <ScrollView style={styles.timeList}>
            {(timeType === 'opening' ? OPENING_TIME_OPTIONS : CLOSING_TIME_OPTIONS).map(time => (
              <TouchableOpacity
                key={time}
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={styles.businessName}>
          {business?.businessName || (businesses.length > 0 ? 'All Businesses' : 'Business')}
        </Text>

        {business && business.addresses.length === 1 ? (
          // Single address form
          <View style={styles.singleAddressForm}>
             ...
             <Text style={styles.checkboxText}>{business.addresses[0].address}</Text>
            </View>
            ...
            <View style={styles.timeRow}>
              <View style={styles.timeColumn}>
                ...
                <TouchableOpacity
                  style={styles.timeSelector}
                  onPress={() => setShowOpeningTimeDropdown('single')}
                >
                  <Text style={[styles.timeSelectorText, !singleBusinessHours.openingTime && styles.placeholder]}>
                    {singleBusinessHours.openingTime || 'Select'}
                  </Text>
                  <Text style={styles.dropdownArrow}>▼</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.timeColumn}>
                ...
                <TouchableOpacity
                  style={styles.timeSelector}
                  onPress={() => setShowClosingTimeDropdown('single')}
                >
                  <Text style={[styles.timeSelectorText, !singleBusinessHours.closingTime && styles.placeholder]}>
                    {singleBusinessHours.closingTime || 'Select'}
                  </Text>
                  <Text style={styles.dropdownArrow}>▼</Text>
                </TouchableOpacity>
              </View>
            ...
            <View style={styles.daysContainer}>
              {DAYS_OF_WEEK.map(day => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.dayButton,
                    singleBusinessHours.days.includes(day) && styles.dayButtonSelected
                  ]}
                  onPress={() => toggleDay(day)}
                >
                  <Text style={[
                    styles.dayButtonText,
                    singleBusinessHours.days.includes(day) && styles.dayButtonTextSelected
                  ]}>
                    {day}
                  </Text>
                </TouchableOpacity>
            ...
            {/* Time Dropdowns for single form */}
            <TimeDropdown
              visible={showOpeningTimeDropdown === 'single'}
              onSelect={(time) => updateTime(null, 'opening', time)}
              onClose={() => setShowOpeningTimeDropdown(null)}
              timeType="opening"
            />
            
            <TimeDropdown
              visible={showClosingTimeDropdown === 'single'}
              onSelect={(time) => updateTime(null, 'closing', time)}
              onClose={() => setShowClosingTimeDropdown(null)}
              timeType="closing"
            />
          </View>
